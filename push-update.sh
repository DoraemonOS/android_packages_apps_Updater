...
updates_dir=/data/doraemon_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have DoraemonOS-VERSION-DATE-TYPE-* as format"
...
# Assume DoraemonOS-VERSION-DATE-TYPE-*.zip
...
adb shell "killall ro.doraemon.updater 2>/dev/null"
adb shell "sqlite3 /data/data/ro.doraemon.updater/databases/updates.db" \
